...
async function getGitHubToken(): Promise<string> {
  ...
  // Newer gh versions support `gh auth token`; older versions do not.
  const directProc = Bun.spawn(["gh", "auth", "token"], { stdout: "pipe", stderr: "pipe" });
  const directStdout = await new Response(directProc.stdout).text();
  const directStderr = await new Response(directProc.stderr).text();
  await directProc.exited;

  const directToken = directStdout.trim();
  if (directToken && !directStderr.includes("unknown command")) {
    return directToken;
  }

  // Compatibility fallback for older gh versions.
  const statusProc = Bun.spawn(["gh", "auth", "status", "-t"], { stdout: "pipe", stderr: "pipe" });
  ...
  const combined = `${statusStdout}\n${statusStderr}`;
  const match = combined.match(/Token:\s+(\S+)/);
  const token = match?.[1]?.trim() ?? "";
  if (!token) {
    throw new Error("No GITHUB_TOKEN and could not extract a token from gh auth output. Run `gh auth login` or set GITHUB_TOKEN.");
  }
  return token;
}
