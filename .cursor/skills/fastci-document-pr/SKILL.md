...
Before running `gh pr create`, determine the correct base branch:
1. If a `.demo-state` file exists in the repo root and contains `BRANCH_NAME`, use that value as the base: `gh pr create --base $BRANCH_NAME ...`
2. Otherwise, omit `--base` and let `gh` use the repository default.

The PR body must include the following sections **in order**.
